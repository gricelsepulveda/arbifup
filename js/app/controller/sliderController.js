app.controller('sliderController', ['$scope', 'slider', '$http', function ($scope, upload , $http,  $location) 
    {
        var tab = 1;
        $scope.selectTab1 = function(tab){
            ...
            url: '/arbifup/administracion/ajax',
            method: "POST",
            data: $.param({'case':7,'slider': $scope.slidertab }),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
            }).success(function (data, status, headers, config) {
                $scope.slider = data;
